...
def predict(req: AudioRequest):
    try:
        ...
        try:
            ...
            raise HTTPException(status_code=400, detail=f"Pydub error: {e}")
...
def multimodal_predict(data: MultimodalRequest):
    try:
        # Use fake values if not provided
        motion = data.motion if data.motion is not None else random.uniform(0, 10)
        heart_rate = data.heart_rate if data.heart_rate is not None else random.uniform(70, 120)
